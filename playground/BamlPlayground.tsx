...
export function BamlPlayground({ compact }: BamlPlaygroundProps = {}) {
        ...
        const runtime = wasm.BamlWasmRuntime.create(
          {
            ...
            env: (variable: string) => {
              if (variable === 'OPENROUTER_API_KEY') {
  ...
  return (
    <div className={`flex flex-col gap-3 w-full ${compact ? 'h-full overflow-hidden' : ''}`}>
      ...
      <div className={`flex flex-col lg:flex-row gap-4 ${compact ? 'flex-1 min-h-0' : 'min-h-[600px]'}`}>
        {/* Editor */}
        <div className="flex-1 min-w-0 flex flex-col gap-3">
          <Card className={`flex-1 flex flex-col overflow-hidden ${compact ? 'min-h-0' : 'min-h-[400px]'}`}>
            <BamlEditor
              value={code}
        ...
        <div className={`w-full lg:w-[420px] flex flex-col gap-3 ${compact ? 'overflow-auto' : ''}`}>
          {/* Run section */}
          {selectedFn && (
            <>
              <Card className="p-3">
              ...
              {(result || error) && (
                <Card className="p-3 overflow-auto max-h-[250px]">
