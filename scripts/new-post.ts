...
import { log as clackLog, confirm, group, intro, isCancel, note, outro, select, tasks, text } from '@clack/prompts'
...
import { Author, OpenGraph, Post } from '~/lib/types'
...
function unwrap(strings: TemplateStringsArray | string, ...expressions: any[]): string {
...
async function getGitUser(): Promise<string> {
  ...
  if (!gitUser) {
    outro(pc.red('Error: Git user name not configured. Please run: git config --global user.name "Your Name"'))
    process.exit(1)
  }
...
async function findExistingAuthors(): Promise<Map<string, Partial<Author> & { name: string }>> {
...
async function getLinkedInProfileImage(linkedinUrl: string): Promise<string | undefined> {
  ...
  await tasks([
    {
      ...
      task: async () => {
        try {
          ...
          const imageMatch = content.stdout.match(/profile-displayphoto-shrink_\d+_\d+\/\d+\/\d+\/\d+\/[^"]+/)
          if (!imageMatch) {
...
async function promptForNewAuthor(gitUser: string): Promise<Partial<Author> & { name: string }> {
  log.info(`Prompting for new author details for: ${gitUser}`)
  const linkedinInput = await text({
    message: 'Enter your LinkedIn URL (optional) Find it here: https://www.linkedin.com',
...
async function selectAuthor(
  ...
  const matchingAuthor = authorsList.find((name) => name.toLowerCase() === gitUser.toLowerCase())
  ...
  const authorChoice = await select({
    ...
    options: [
      ...
      ...(!matchingAuthor ? [{ value: 'new', label: `+ Add new author (${gitUser}) - found from git config` }] : []),
      ...authorsList.map((a) => ({ value: a, label: a })),
    ],
  })
  ...
  return choice === 'new' ? await promptForNewAuthor(gitUser) : authors.get(choice)!
}

async function promptForOg(postTitle: string, postDescription: string): Promise<OpenGraph | undefined> {
  note(
    pc.white(
  ...
  if (!isCancel(useCustomOg) && useCustomOg) {
    ...
    const ogDescriptionInput = await text({
      message: 'Enter the OG description (optional, press Enter to use post description):',
    ...
    const ogImageInput = await text({
      message: 'Enter the OG image path (relative to public directory) (optional):',
...
async function createDirectories(date: string, slug: string): Promise<{ postsDir: string; mediaDir?: string }> {
  ...
  if (createMediaDir) {
    ...
    return { postsDir, mediaDir: mediaPath }
  }
    ...
    const postInfo = await group(
      {
        title: () =>
          text({
            ...
            validate: (value) => (!value ? 'Please provide a title' : undefined),
          }),
        description: () =>
          text({
            message: 'Enter a brief description:',
            placeholder: 'A short description of your blog post',
            validate: (value) => (!value ? 'Please provide a description' : undefined),
          }),
        type: () =>
          select({
        ...
        ? [
            {
              ...
              task: async () => {
                const fullMediaDir = path.join(process.cwd(), 'public', mediaDir)
...
✅ Successfully created:
   - Post: ${pc.blue(mdxPath)}${mediaDir ? `\n   - Media directory: ${pc.blue(mediaDir)}` : ''}${
    ...
    throw new Error(`Failed during ${currentStep}: ${err instanceof Error ? err.message : String(err)}`)
  }
}

generatePost().catch((err: unknown) => {
  const errorMessage = err instanceof Error ? err.message : String(err)
  console.error('Error occurred. Re-run with -v flag for verbose output to debug the issue.')
