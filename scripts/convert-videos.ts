...
async function findVideoFiles(directory?: string): Promise<string[]> {
  ...
  if (!searchDir) {
    ...
    if (latestMediaDir) {
      options.push({ value: latestMediaDir, label: `Latest blog post (${latestMediaDir})` })
    }

    // Add current directory second
    options.push({ value: currentDir, label: `Current directory (${currentDir})` })
  ...
  const options = await group(
    {
      ...
      crf: () =>
        text({
          ...
          validate: (value) => {
            ...
            if (isNaN(num) || num < 15 || num > 35) return 'Please enter a number between 15 and 35'
            return undefined
          },
        }),
      bitrate: () =>
      ...
      scale: () =>
        text({
          ...
          placeholder: '1280:-1',
          initialValue: '1280:-1',
          validate: (value) => (!value ? 'Please provide scaling dimensions' : undefined),
        }),
      audio: () =>
        confirm({
...
async function convertVideo(file: string, options: FfmpegOptions): Promise<ConversionResult> {
...
function createFileSelection(filePath: string): FileSelection {
  ...
  const webmPath = path.join(path.dirname(filePath), path.basename(filePath, '.mp4') + '.webm')
  return {
    filePath,
    fileName,
...
async function selectFilesToConvert(videoFiles: string[]): Promise<string[]> {
  const fileSelections = videoFiles.map(createFileSelection).sort((a, b) => a.fileName.localeCompare(b.fileName))
...
async function handleExistingFiles(filesToConvert: string[]): Promise<string[]> {
  const fileSelections = filesToConvert.map(createFileSelection).sort((a, b) => a.fileName.localeCompare(b.fileName))
  ...
  const filesToOverride = await multiselect({
    message: 'The following files already exist. Select which ones to override:',
  ...
  return filesToConvert
    ...
    .filter((file) => {
      const webmPath = path.join(path.dirname(file), path.basename(file, '.mp4') + '.webm')
      return !existsSync(webmPath) || filesToOverrideSet.has(file)
    })
}

async function processBatch(files: string[], ffmpegOptions: FfmpegOptions, batchSize: number) {
  ...
  for (let i = 0; i < sortedFiles.length; i += batchSize) {
    ...
    s.start(`Converting batch ${pc.dim(`[${batchFileNames.join(', ')}]`)} of ${totalFiles} files...`)
    ...
    try {
      ...
      results
        .sort((a, b) => {
          const aName = a.success ? path.basename(a.result.inputFile) : path.basename(a.file)
          const bName = b.success ? path.basename(b.result.inputFile) : path.basename(b.file)
          return aName.localeCompare(bName)
        })
        .forEach((result) => {
          if (result.success) {
            const { inputFile, outputFile, inputSize, outputSize, compressionPercent } = result.result
            log.message(
            ...
            log.error(`${pc.red('✖')} ${path.basename(result.file)}: ${result.error}`)
          }
        })
    } catch (error) {
      s.stop(`${pc.red('Batch conversion failed')}: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
}
...
Press Ctrl+C to cancel.`,
  ...
  const batchSizeInput = await text({
    ...
    validate: (value) => {
      ...
      if (num > filesToConvert.length) return `Maximum batch size is ${filesToConvert.length}`
      if (num > cpuCount)
        return pc.yellow(`Warning: Using more processes than available CPUs (${cpuCount}) may slow down your system`)
      return undefined
    },
  })
