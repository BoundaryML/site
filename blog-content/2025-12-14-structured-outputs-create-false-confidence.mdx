...
- you need a way to embed the output format in the prompt
- you need a parser that can find JSON in your output and can handle case1, case2, case3
- you need a parser that can coerce the JSON into your output schema, if the model, say, returns a float where you wanted an int

We call this [schema-aligned parsing](https://boundaryml.com/blog/schema-aligned-parsing#what-is-sap-and-why-does-it-work-so-well), and have made it available in BAML, our open-source, local-only DSL.

This is [why BAML - our open-source, local-only DSL - uses schema-aligned parsing](https://docs.boundaryml.com/guide/introduction/why-baml#3-schema-aligned-parsing-sap): we believe letting the LLM respond in as natural a fashion as possible is the most effective way to get the highest quality response from it. For an example of this in action, take a look at this writeup from Kevin Madura about [improving extraction 
quality by 20%](https://kmad.ai/DSPy-Optimization).

# Clarifications

In response to a lot of the comments we've gotten so far:

To be more precise about my claim, it's not that constrained decoding _always_ gives definitively bad outputs (I went a little too click-baitey with my original tagline, I admit it), so much as that it's really easy, with constrained decoding, to create the illusion that you're getting good responses.
...
(This is a very hard message to convey at the start of an article, and depends on a lot of nuance that needs to first be explained with examples.)
...
- "Let Me Speak Freely" is a fundamentally flawed paper.
  ...
  I hadn't seen dottxt's ["Say What You Mean"](https://blog.dottxt.ai/say-what-you-mean.html) response to the "Let Me Speak Freely" paper before, and their critique that the "Let Me Speak Freely" paper compares apples to oranges is entirely valid.
