...
import { unwrap, atomWithStorage, atomFamily, useAtomCallback } from 'jotai/utils'
...
export const lessonAndPageIdAtom = atom(
    ...
    (get, set, update: { lessonIndex: number; pageIndex: number; tabId: string }) => {
        const lessons = get(lessonsAtom)
        
        // Auto-correct indices to ensure they're always valid
        const safeLessonIndex = Math.max(0, Math.min(update.lessonIndex, lessons.length - 1))
        const lesson = lessons[safeLessonIndex]
        const safePageIndex = lesson ? Math.max(0, Math.min(update.pageIndex, lesson.pages.length - 1)) : 0
        const page = lesson?.pages[safePageIndex]
        
        // Ensure tab ID exists in the current page, fall back to defaultTab if not
        const availableTabs = page?.fileContents ? Object.keys(page.fileContents) : []
        const safeTabId = availableTabs.includes(update.tabId) ? update.tabId : (page?.defaultTab || availableTabs[0] || '')
...
export const safeCurrentIndicesAtom = atom((get) => {
    ...
    const safePageIndex = lesson ? Math.max(0, Math.min(pageIndex, lesson.pages.length - 1)) : 0
    const page = lesson?.pages[safePageIndex]
    
    // Ensure tab ID exists in the current page, fall back to defaultTab if not
    const availableTabs = page?.fileContents ? Object.keys(page.fileContents) : []
    const safeTabId = availableTabs.includes(tabId) ? tabId : (page?.defaultTab || availableTabs[0] || '')
...
export const visibleFileAtom = atom((get): ICodeBlock => {
    const { lessonIndex, pageIndex, tabId, lesson, page } = get(safeCurrentIndicesAtom)
    console.log("lessonIndex", lessonIndex)
    console.log("pageIndex", pageIndex)
    console.log("tabId", tabId)
    const code = page?.fileContents?.[tabId as keyof typeof page.fileContents] || ''
...
function getLanguageFromFilename(filename: string): 'python' | 'typescript' | 'baml' {
