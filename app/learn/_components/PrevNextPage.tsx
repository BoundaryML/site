...
import { lessonsAtom, lessonAndPageIdAtom, safeCurrentIndicesAtom } from "../lib/learn-atoms"
...
export default function PrevNextPage() {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (target.tagName === 'INPUT' || 
                ...
                target.closest('.cm-editor')) { // Also ignore if in CodeMirror editor
                return
            }
    ...
    function nextPage() {
        ...
        const onLastPageOfLesson = lesson && lessonAndPageId.pageIndex === lesson.pages.length - 1
        if (onLastPageOfLesson) {
            if (!onLastLesson) {
                const newLessonIndex = Math.min(lessons.length - 1, lessonAndPageId.lessonIndex + 1)
                const newLesson = lessons[newLessonIndex]
                setLessonAndPageId({
                    lessonIndex: newLessonIndex,
                    pageIndex: 0,
            ...
            const newPageIndex = Math.min((lesson?.pages?.length || 100) - 1, lessonAndPageId.pageIndex + 1)
            const newPage = lesson?.pages?.[newPageIndex]
            setLessonAndPageId({
                lessonIndex: lessonAndPageId.lessonIndex,
                pageIndex: newPageIndex,
    ...
    return (
        <div className="flex flex-row gap-2 w-full justify-center text-2xl">
            ...
            <span className="w-2">{lessonAndPageId.pageIndex + 1}</span> <span className="w-2">/</span> <span className="w-2">{pagesInLesson}</span>
            <span
              className={linkStyle}
              onClick={() => nextPage() }>&gt;
            </span>
