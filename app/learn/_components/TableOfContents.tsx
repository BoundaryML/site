import { currentLessonAndPage, lessonAndPageIdAtom, lessonsAtom } from "../lib/learn-atoms";
...
export default function TableOfContents() {
    ...
    function gotoPage(lessonId: number, pageId: number) {
        if (lessons &&
            lessonId >= 0 && lessonId < lessons.length &&  
            pageId >= 0 &&
            lessons[lessonId] &&
    ...
    const extraLessonStyle = (index: number) => index === lessonAndPageId.lessonIndex ? "bg-blue-100" : ""
    const extraPageStyle = (lessonIndex: number, pageIndex: number) =>
        pageIndex === lessonAndPageId.pageIndex && lessonIndex == lessonAndPageId.lessonIndex ? "bg-blue-200": ""
    return (
        <div className="p-2">
                ...
                <div key={`lesson-${index}-${lesson.title}`}>
                    ...
                    {lesson.pages.map((page, pageIndex) => (
                        <div
                         ...
                         >
                            <h2
                              onClick={ () => gotoPage(index, pageIndex)}
                            >{page.title}</h2>
                        </div>
                    ))}
                </div>
